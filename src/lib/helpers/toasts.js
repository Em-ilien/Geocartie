...
export function onQuizzFalseAnswer(newInstruction = null) {
    if (newInstruction === null)
        ...
        toast.error(`Vous avez cliqué sur un autre département. À présent cherchez ${newInstruction.prefix}${newInstruction.name} (${newInstruction.id})`, {
            style: STYLE,
            position: QUIZZ_POSITION,
            duration: 5000,
        });
...
export function initQuizz(instruction) {
    ...
    toast.success(`Bienvenue dans le quizz ! Cherchez le département ${instruction.prefix}${instruction.name} (${instruction.id})`, {
        style: STYLE,
        position: QUIZZ_POSITION,
        duration: 5000,
    });
